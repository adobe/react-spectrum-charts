...
Read `.claude/architecture-core.md`, plus `.claude/architecture-child-dispatch.md` (the sanitize gate — all 6 sanitize functions — and the three dispatch patterns: direct cast, adapter call, recursive) and `.claude/architecture-file-creation.md`. If the component exists in S2, also read `.claude/architecture-s2-parity.md`.
