...
## Run-Epic Subagent Mode

When this skill is invoked from `run-epic` as a background subagent, the agent **cannot interact
with the user**. Phase 3's "wait for user response" step is skipped — instead, all proposed
solutions are returned in the structured output and the orchestrator presents them to the user.

**Phase 3 behavior in subagent mode:** Complete the full 2–3 solution proposals as specified, but
do NOT pause or ask a question. Proceed directly to Phase 4 using the solution you recommend.

**Required output format** (append to the end of the response):
...
SOLUTION_1:
  NAME: <short label>
  SUMMARY: <2-3 sentences: what it does, how it hooks into the pipeline>
  KEY_FILES: <comma-separated list of files that would change>
  VEGA_FEASIBILITY: <yes + explanation | no + explanation>
  TRADEOFFS: <complexity, reuse of existing patterns, risk>
SOLUTION_2:
  NAME: <short label>
  SUMMARY: <...>
  ...
  TRADEOFFS: <...>
SOLUTION_3:
  NAME: <short label — omit if only 2 viable approaches>
  ...
RECOMMENDATION: <solution name and 1-2 sentence rationale>
PROTOTYPE_PATH: <file path of the Vega spec or story produced, if any>
IMPLEMENTATION_SKILL: <which skill file a follow-on implementation should use>
---END-RESULT---
```

The orchestrator reads `PROPOSED_SOLUTIONS` to present choices to the user, captures the decision,
and writes `planning/<epic-key>/spikes/<task-id>.md` with the chosen approach before continuing.
