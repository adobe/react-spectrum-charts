...
describe('getTrendlineDimensionRangeTransforms()', () => {
  test('should add filters if the dimensionRange has non-null values', () => {
    ...
    expect(getTrendlineDimensionRangeTransforms(DEFAULT_TIME_DIMENSION, [null, 2])[0].expr.split(' && ')).toHaveLength(
      1
    );
    expect(getTrendlineDimensionRangeTransforms(DEFAULT_TIME_DIMENSION, [1, null])[0].expr.split(' && ')).toHaveLength(
      1
    );
