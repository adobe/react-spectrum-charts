...
export const getBarColorEncoding = (options: BarSpecOptions): ColorValueRef | ProductionRule<ColorValueRef> => {
  ...
  if (colorFromData && typeof color === 'string') {
    const fallback = getColorValue('gray-500', colorScheme);
    return [
 ...
 * When colorFromData is true, fill uses datum[color] directly instead of the color scale.
 */
...
/**
 * Returns stroke color production rules. When colorFromData is true, default stroke uses datum[color].
 * Selected/popover state still uses the theme highlight color (static-blue).
 */
