...
describe('Title', () => {
  test('Basic renders properly', async () => {
    ...
    expect(title).toHaveAttribute('font-weight', '700'); // S2 default is 700
    expect(title).toHaveAttribute('text-anchor', 'start'); // S2 default is 'start'
  ...
  test('Orient renders properly', async () => {
    ...
    expect(title).toHaveAttribute('transform', 'translate(0,19)'); // S2 fontSize is 24 (vs 18), affecting positioning

    const titleGroups = getAllMarksByGroupName(chart, 'role-title', 'g');
    const positioningGroup = titleGroups[0];
    expect(positioningGroup).toHaveAttribute('transform', 'translate(0,369)'); // S2 default position is 'start' (0), larger fontSize affects Y position
  });
