...
describe('buildAxisStructure()', () => {
  describe('categorical', () => {
    ...
    test('does not wrap the last tick back to the first', () => {
      const { structure } = buildAxisStructure({ data, field: 'browser', type: 'categorical' });
      const hasEdge = Object.values(structure.edges).some(
        (edge) => (edge.source === 'Safari' && edge.target === 'Chrome') || (edge.source === 'Chrome' && edge.target === 'Safari')
      );
