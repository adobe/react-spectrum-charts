...
describe('composeRegions()', () => {
  ...
  test('chains region entry points together in order with left/right/up/down sibling edges', () => {
    const content = buildBarStructure({ data, dimension: 'browser' });
    const xAxis = buildAxisStructure({ data, field: 'browser', type: 'categorical' });
    const third = buildAxisStructure({ data, field: 'browser', type: 'categorical' });
    const composed = composeRegions([
      { name: 'content', structure: content.structure, entryPoint: content.entryPoint, namespace: false },
      { name: 'xAxis', structure: xAxis.structure, entryPoint: xAxis.entryPoint },
      { name: 'third', structure: third.structure, entryPoint: third.entryPoint },
    ]);
    ...
    const contentToAxis = contentEdges.find((edge) => edge.target === xAxisRootId);
    expect(contentToAxis?.navigationRules).toEqual(expect.arrayContaining(['left', 'right', 'up', 'down']));
  });

  test('does not wrap the last region back to the first', () => {
    const content = buildBarStructure({ data, dimension: 'browser' });
    const xAxis = buildAxisStructure({ data, field: 'browser', type: 'categorical' });
    const third = buildAxisStructure({ data, field: 'browser', type: 'categorical' });
    const composed = composeRegions([
    ...
    const thirdRootId = `third::${third.entryPoint}`;
    const thirdRoot = composed.structure.nodes[thirdRootId];
    const thirdEdges = thirdRoot.edges.map((id) => composed.structure.edges[id]);
    expect(thirdEdges.some((edge) => edge.target === content.entryPoint)).toBe(false);
  });
