...
describe('Chart spec builder', () => {
  ...
  describe('getLinearColorScale()', () => {
    ...
    test('should transform custom color names', () => {
      ...
      expect(getLinearColorScale(['blue-200', 'rgb(20, 30, 40)', 'static-black'], 'light')).toHaveProperty('range', [
        colors['blue-200'],
        'rgb(20, 30, 40)',
        'static-black', // Pass through non-spectrum colors
      ]);
    });
  });
  ...
  describe('getTwoDimensionalColorScheme()', () => {
    ...
    test('should get colors from color names for light and dark mode', () => {
      expect(getTwoDimensionalColorScheme(['blue-400', 'blue-500', 'blue-600', 'blue-700'], 'light')).toStrictEqual([
        ['#ACCFFD'],
        ['#8EB9FC'],
        ['#729EFD'],
        ['#5D89FF'],
      ]);
      expect(getTwoDimensionalColorScheme(['blue-400', 'blue-500', 'blue-600', 'blue-700'], 'dark')).toStrictEqual([
        ['#122D9A'],
        ['#1A3AC3'],
        ['#2549E5'],
        ['#345BF8'],
    ...
    test('should convert color scheme in second dimension to correct colors', () => {
      expect(getTwoDimensionalColorScheme(['categorical6', 'divergentOrangeYellowSeafoam5'], 'light')).toStrictEqual([
        [
          'rgb(15, 181, 174)',
          'rgb(64, 70, 202)',
          'rgb(246, 133, 17)',
