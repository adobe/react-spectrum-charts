...
const getAxisAnnotationDetailData = (name: string, options, colorScheme): ValuesData => {
  const newOptions = options.map((option) => ({ ...option, color: getS2ColorValue(option.color, colorScheme) }));
...
const getAxisAnnotationAggregateData = (name, dataKey, color, colorScheme): SourceData => {
  return {
    ...
    transform: [
      ...
      {
        type: 'formula',
        expr: `datum.number > 1 || datum.color == null ? '${getS2ColorValue(color, colorScheme)}' : datum.color`,
        as: 'color',
      },
    ],
  };
